function [dop,okay,msg] = dopPlot_Err(dop_input,varargin)
% dopOSCCI3: dopPlot
%
                    ...
                    if okay
                        for i = 1 : numel(dop.data.epoch_labels) % size(dop.tmp.plot_data,3)
                            ...
                            %dop.tmp.(dop.tmp.name).h = plot(dop.epoch.times,...
                                ...
                                participants = size(dop.tmp.data,2);
                                dop.tmp.err_data = std(squeeze(dop.tmp.data(:,dop.plot.screen,:)),0,2);
                                dop.tmp.SE_data = dop.tmp.err_data/sqrt(participants);
                                lineProps = {'b','r',[17 17 17]};
                                
                            dop.tmp.(dop.tmp.name).h = shadedErrorBar(dop.epoch.times,...
                                mean(squeeze(dop.tmp.plot_data(:,i)),2),...
                                squeeze(dop.tmp.SE_data(:,i)),...
                                lineProps{i});
                        ...
                        for i = 1:numel(dop.data.epoch_labels)
                            if i == 1 && ishold(dop.fig.h); hold; end
                            dop.tmp.name = dop.data.epoch_labels{i};
                            dop.tmp.(dop.tmp.name).h 
                        end
